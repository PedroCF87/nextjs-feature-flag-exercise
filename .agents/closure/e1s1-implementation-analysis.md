...
| 10 | [`client/src/components/flags-filter-controls.tsx`](../../client/src/components/flags-filter-controls.tsx) (**new**) | AC-9 and AC-10 require dedicated UI for filter input, active-filter indication, and a "Clear all filters" action. This is a new component responsible for rendering the filter form and communicating state changes to the parent. | **Create new.** Render inputs for `environment`, `status`, `type`, `owner`, and `name`. Emit filter state via `onChange` callback. Display active-filter indicator when any filter is non-empty. Expose a "Clear all filters" button that resets all inputs. |
